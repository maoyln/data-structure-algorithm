...
function maxCrossingSum(arr, left, mid, right) {
  ...
  sum = 0;
  var rightSum = 0; // 包括中点的右半部分的最大子列和

  for (var _i = mid + 1; _i <= right; _i++) {
    sum += arr[_i];
...
function maxSubArraySum(arr, left, right) {
  ...
  var mid = Math.floor((left + right) / 2); // 返回以下三者中的最大值：
  // 1. 左半部分的最大子列和
  // 2. 右半部分的最大子列和
  // 3. 跨越中点的最大子列和

  return Math.max(maxSubArraySum(arr, left, mid), maxSubArraySum(arr, mid + 1, right), maxCrossingSum(arr, left, mid, right));
} // 示例
